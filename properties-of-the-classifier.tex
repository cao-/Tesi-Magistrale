...
\label[flatlimit]
\preskip
\property  For a fixed $F$ the vector $\Bm Q\,= \Bm Q_{\.F\!}\,$, which depends on the shape parameter~$ε$, converges to a limit vector as $ε$ approaches~$0$.
\proof
...
Although, as previously stated, for a fixed value of~$ε$ the vector~$\Bm Q$ is not invariant under a translation applied to~$F$, fortunately the translational invariance is achieved in the flat limit.\fnote{The limiting case when~$ε\to 0$ is usually called {\em flat limit}, because the basis functions generated by~$\phi_ε$ become increasingly flat as~$ε\to 0$.}
\preskip
\property If $G = F+η$ for some~$η\in\R$, then $\Bm Q_{\.G\!} \to \Bm Q_{\.F}$ when~$ε\to 0$.
\proof
