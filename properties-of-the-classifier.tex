...
G = \{\.\range g_N/\} 
  ...
  \eqcolon γ F + η.
$$
If $X$ is fixed, in order to distinguish the interpolant of $(X,F)$ from the interpolant of~$(X, G)$ we'll denote them as $s_{F}$ and~$s_{G\!}$ respectively; the same for the vector~$\Bm Q\,$, which will appear as $\Bm Q_{\.F\!}$ in the first case and~$\Bm Q_{\.G\!}$ in the second case.
...
$$
whose coefficients~$a_{n\!}(r)$ are explicit\fnote{They can actually be computed from the expression of~$\phi$.} functions of~$r$.
...
$$
\proof
Assume by now that $F$ is not constant (meaning that $f_i\neq f_j$ for at least a pair of indices~$i,j$), so that in particular $s_F$ and~$s_G$ are different from the zero function, and the elements $(Q_F)_k$ and~$(Q_G)_k$ are defined as
$$
(Q_F)_k = {(U_F)_k\over\norm{s_F}_\phi^2}
...
If we show that~$\CC_{\!\!\Bm{--p}} \.\Bm 1 = \Bm 1^{\!T\!}\.\CC_{\!\!\Bm{--p}\!} = \Bm 0$, then this limit is~$1$ and expression~\ref[differenceterm] is equal to~$0$. Then it follows that~$\lim_{ε\to 0} \Bm Q_{\.G\!} = \lim_{ε\to 0} \Bm Q_{\.F}$.  The fact that the sum of the rows and the columns of the matrix~$\CC_{\!\!\Bm{--p}}$ is zero can be shown by expanding $\Bm A$ and~$\Bm C$ using the power series~\ref[powerA] and the Laurent series~\ref[laurentC]. In fact, the multiplication of $\Bm A$ by~$\Bm C$ yields
$$
\displaylines{
(\CA_{\Bm 0} + \CA_{\Bm 1}\,ε + \CA_{\Bm 2}\,ε^2+\dots)(\CC_{\!\!\Bm{--p}}\,ε^{-p} + \CC_{\!\!\Bm{--p+}\Bm 1}\,ε^{-p+1} + \CC_{\!\!\Bm{--p+}\Bm 2}\,ε^{-p+2} + \dots) = \Bm I\cr
(\CA_{\Bm 0}\CC_{\!\!\Bm{--p}})\,ε^{-p} + (\CA_{\Bm 0} \CC_{\!\!\Bm{--p+}\Bm 1} + \CA_{\Bm 1}\CC_{\!\!\Bm{--p+}\Bm 1})\,ε^{-p+1} + \dots = \Bm I. 
}
$$
