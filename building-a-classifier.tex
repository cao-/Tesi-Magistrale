...
\preskip
\definition
Given a set~$X = \{\range x_N/\}\subset\Omega\subseteq\R^d$ of data sites, with $N ≥ 2$, a set $F = \{\range f_N/\}$ of function values, and a radial kernel~$\phi$ which is positive definite on~$\R^d$, we define the vector
$$
...
Of course it is impossible to tell with certainty how regular a function~$f$ is, just by looking at a set of values sampled from it and without any other assumption about the function itself.  In fact, $f$~can have an arbitrary behaviour at the other unseen points. The meaning of this classifier is therefore more like saying if the sample is more likely to come from a function which is regular (value~$1$) or a function which isn't (value~$-1$).  The regularity is inspected by the kernel function $\phi$ and the tolerance~$\tau$ is the decision boundary between regular and not regular functions, that must be set according to how much evidence we need for the classification. The rationale behind the above given definition for the classifier~$\Cal R$ should become clear in the following section.
