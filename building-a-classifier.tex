...
In what follows, we will always assume that  the kernel~$Φ$ is radial, i.e., of the form~\ref[phi] for some univariate positive definite function~$\phi$. Let $X=\{\range x_N/\}$ be a set of data sites and $f$ a function that is no more assumed to belong to the native space of~$Φ$.  Suppose that $f$ has been sampled at~$X$ and the sampling is dense enough to catch the relevant features of~$f$ on its considered domain.  Our goal is to recognise whether or not $f$ is discontinuous just by looking at~$X$ and the corresponding sampled function values~$f(X)$.
...
$$
whose elements are defined as~$E_k\coloneq f(x_k)-s^{(k)}(x_k)$, and which is typically used to find the appropriate shape parameter~$ε$ for the interpolation with  a positive definite radial function~$\phi_ε$. The value of~$ε$ is chosen such that the $p$-norm
$$
...
\preskip
\theorem[(Rippa)]
Let $X=\{\range x_N/\}$ be a set of data sites, $Φ:\,\Omega\times\Omega\to\R$ a symmetric positive definite kernel, and
$$
...
E_k = {α_k\over C_{k,k}},
$$
where $\Bm C$ is the inverse of the interpolation matrix~$\Bm A=\Bm A_{Φ,X}$ associated to the kernel~$Φ$ and the set~$X$ of locations, and $\Bm α$ is the vector of coefficients of the unique function~$s\in\Cal S_{Φ,X}$ that interpolates $f$ on~$X$, that is the solution of the linear sistem~$\Bm{Aα} = \Bm f$. 
\proof
...
$$
\Bm t(x)\coloneq (Φ(x,x_1), Φ(x,x_2), \dots, Φ(x, x_N))^T.
$$
Define then, for each~$k\in\{1,2,\dots, N\}$, the vectors $\Bm t^{(k)}\!(x)$ and~$\Bm f\,^{(k)}$ of length~$N-1$ by removing the $k$-th element from the vectors $\Bm t(x)$ and~$\Bm f$ respectively.  Similarly, define the matrix~$\Bm A^{(k)}$ of size~$(N-1)\times(N-1)$ by removing from~$\Bm A$ both its $k$-th~row and $k$-th~column.  If $\Bm β^{\lbrace k\rbrace}$ is the solution of the linear system~$\Bm A^{(k)}\Bm β^{\lbrace k\rbrace} = \Bm f\,^{(k)}$, then the function~$s^{(k)}\in \Cal S_{Φ,X^{(k)}}$ that interpolates~$f$ on the restricted set~$X^{(k)}$ of data sites can be expressed as
$$
...
$$
must hold, since $\Bm C$ is defined as the inverse of~$A$.  Notice that if $\Bm y$ and~$\Bm z$ are any two vectors  of length~$N-1$ such that~$y_k = 0$, and if $\Bm y^{(k)}$ and~$\Bm z^{(k)}$ are the same vectors without the $k$-th element, then
$$
...
$$
\Bm A^{(k)}\,(C^{[k]}_1,\dots,\rlap{\,\typoscale[1900/]\it/} C^{[k]}_k,\dots, C^{[k]}_N)^T =  \hbox{\bf 0},
$$
...
$$
\Bm A\,(b^{\lbrace k\rbrace}_1,\dots,\rlap{\typoscale[1900/]\it/} b^{\lbrace k\rbrace}_k,\dots, b^{\lbrace k\rbrace}_N)^T= \Bm f\,^{(k)}.
$$
...
$$
\Bm β^{\lbrace k\rbrace} = (b^{\lbrace k\rbrace}_1,\dots,\rlap{\typoscale[1900/]\it/} b^{\lbrace k\rbrace}_k,\dots, b^{\lbrace k\rbrace}_N)^T.
$$
