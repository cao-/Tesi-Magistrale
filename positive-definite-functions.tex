...
Finally we notice that all the  positive definite radial functions~$\phi:[0,\infty)\to\R$ can be rescaled by means of a {\em shape parameter}~$\varepsilon$.  In fact, if the $\varepsilon$-scaled version~$\phi_\varepsilon$ of~$\phi$ is defined as
$$
\phi_\varepsilon(r) = \phi(\varepsilon r),
$$
then $\phi_\varepsilon$ is still positive definite and radial.  When interpolating a sampled function by using a positive definite radial function, the proper choice of the shape parameter has great impact on the accuracy of the result.  There are various techniques that allows for an automatic correct choice of the shape parameter, the most notable being the {\em leave-one-out cross-validation} method (LOOCV) (see Rippa~\cite[rippa_1999] and Fassauer~\cite[fasshauer_2011], section~5).  
