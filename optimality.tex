...
First of all the error~$f-s_{X, f}$ is {\em orthogonal} to~$\Cal S_{Φ, X}$, where the orthogonality is intended in the sense of the scalar product~$\form\cdot\cdot_Φ$ of the native space.  What we mean is that the scalar product between $f-s_{X, f}$ and any function~$h\in\Cal S_{X,f}$ is~zero.  In fact,
$$
...
$$
\norm{f-s_{X,f}}_Φ \leq \norm{f-h}_Φ\mathbox{,\quad for all $h\in\Cal S_{Φ,X}$.}
$$
...
$$
f = s_{X,f} + \epsilon_x(f).
$$
The orthogonality of the two components implies the following {\em Pythagorean identity},
$$
\norm{f}_Φ^2=\norm{s_{f,X}}_Φ^2+\norm{\epsilon_x(f)}^2.
$$ 


Up to now we have considered as interpolant of the pair of data~$(X,f(X))$ only the  unique function~$s_{X,f}$ that belongs to~$\Cal S_{Φ,X}=\langle\,Φ(\cdot,x_j)\, :\, x_j\in X\,\rangle$.  Nonetheless, in the rest of~$\Cal N_Φ$ there could be also other functions which agree with the values~$f(X)$ at the data sites~$X$---one trivial example is $f$ itself.  We are about to see a property satisfied by~$s_{X,f}$,  which makes it special among all the interpolants.
...
$$
\proof
By using the trick of adding and subtracting a function~$g\in\Cal I_{X, f}$, we can expand the square of the norm of~$s_{X,f}$ as follows,
$$
\eqalign{\norm{s_{X,f}}_Φ^2 &= \form{s_{X,f}}{s_{X,f}}_Φ          \cr
						 &= \form{s_{X,f}}{s_{X,f}-g+g}_Φ  \cr
						 &= \form{s_{X,f}}{s_{X,f}-g}_Φ + \form{s_{X,f}}{g}_Φ  \cr
}
$$
Similarly as in~\ref[orthog], the term $\form{s_{X,f}}{s_{X,f}-g}_Φ$ is shown to be zero; by applying the Cauchy-Shwarz inequality to the other term we get,
$$
\norm{s_{X,f}}_Φ^2=\form{s_{X,f}}{g}_Φ\leq\norm{s_{X, f}}_Φ\norm g_Φ,\eqno (\lower.5ex\hbox{*})
$$
and hence that
$$
\norm{s_{X, f}}_Φ \leq \norm{g}_Φ\mathbox{,\quad for all $g\in\Cal I_{X,f}$.}
$$
 Notice that in~$(\lower.5ex\hbox{*})$ equality holds if and only if $g$ and $s_{X,f}$ are linearly dependent.  Since we know that~$\Cal S_{Φ, X}\cap\Cal I_{X, f} = \{s_{X,f}\}$, the only case in which equality  holds in~$(\lower.5ex\hbox{*})$ is when~$g=s_{X,f}$.   This means that the minimum of the norm is realised {\em only} by~$s_{X, f}$. \hfill\QED
%\postskip
