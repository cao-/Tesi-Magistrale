...
$$
s_{X,f} = \sum_{k = 1}^N\alpha_k Φ(\cdot, x_k),  \qquad
s_{X\cup\{x\}, f} = \sum_{k=1}^N\beta_k Φ(\cdot, x_k) + \gamma\, Φ(\cdot, x),
$$
for some coefficients~$\{\alpha_k\}_{k\in\{1,2,\dots,N\}}$, $\{\.\beta_k\}_{k\in\{1,2,\dots,N\}}$ and~$\gamma$.
