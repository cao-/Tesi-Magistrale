...
Instead, in the opposite case, that is when $ε→∞$, the vector~$\Bm Q$ loses its dependence on~$X$.  In fact, if we assume without loss of generality that~$\phi(0)=1$, then the interpolation matrix~$\Bm A$ converges to the identity matrix  when $ε→∞$, and this implies that $\norm{s}_\phi^2→ \Bm f^{\,\.T\!}\Bm f$ and $\norm{s^{(k)\!}}_\phi^2→ (\,\Bm f^{\,\.(k)\!})^{T\!}\Bm f^{\,\.(k)}\!$, and finally that $\smash{Q_k→ f_k^{\.2}/(\,\Bm f^{\,\.T\!}\Bm f\,)}$ (here the vectors $\Bm f$ and~$\smash{\Bm f^{\,\.(k)}\!}$ are defined as in Theorem~\ref[rippatheo]).
...
$$
As proved in Theorem~\ref[flatlimit], when $ε→0$ these two quantities converge,
$$
...
$$
In this equation, if we multiply both terms by~$ε^{-p\!}$ and then take the limit~$ε→0$, we obtain
$$
...
$$
If $X$ becomes~$Y = rX$, then each element~$(A_X)_{i,j}$ becomes
$$
(A_Y)_{i,j} = \phi(ε\.\norm{r\.x_i - r\.x_j}) = \phi(r\.ε\.\norm{x_i - x_j}).
$$
The constant~$r$ uniformly multiplies every occurrence of the parameter~$ε$ in the transformed matrix~$\Bm A_Y$, therefore the change made by~$r$ is irrelevant when the limit~$ε→0$ is considered.~\QED
\postskip

Actually, as it is clear from the proof, the scaling parameter~$r$ could also have been taken negative.  But the change of sign (the symmetry) is already included in the next result.
...
Summing up, we saw that when $ε→0$ the regularity vector~$\Bm Q$ converges to a limit vector which is invariant under a shape preserving transformation applied to~$X$ or to~$F$.  This fact is very important if we wish to employ~$\Bm Q$ to determine the regularity of some data~$(X, F)$---the regularity of two sets of data that have the same shape should be the same.  Figure~\ref[shapefig] may make this claim apparent.
...
\kern-6ex
\caption/f
Three similar sets of data~$(X, F)$, with $X\subset\R$.  Intuitively they all have the same regularity, whatever this means.  Indeed, we showed that in the flat limit the regularity vector~$\Bm Q$ is the same for all of them.
\bigskip
\endinsert
