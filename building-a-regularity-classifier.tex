...
The ideas developed in Section~\ref[addingsec] can be used to try and classify samples of functions by their regularity.  The regularity classifier that we are going to define will be the main tool necessary to later develop an algorithm for the interpolation of discontinuous functions.
...
E_k = {α_k\over C_{k,k}},
$$
where $\Bm C$ is the inverse of the interpolation matrix~$\Bm A=\Bm A_{\phi,X}$ associated to the function~$\phi$ and the set~$X$ of locations, and $\Bm α$ is the vector of coefficients of the unique function~$s\in\Cal S_{\phi,X}$ that interpolates $f$ on~$X$, that is the solution of the linear sistem~$\Bm{Aα} = \Bm f$. 
\proof
...
$$
\Bm β^{\lbrace k\rbrace\!} = (b^{\lbrace k\rbrace}_1,\dots,\rlap{$\!$\typoscale[1700/]$\backslash$} b^{\lbrace k\rbrace}_k,\dots, b^{\lbrace k\rbrace}_N)^T. \eqmark[rippabeta]
$$
