...
The ideas developed in the previous section can be used to try and classify samples of functions by their regularity.  The regularity classifier that we are going to define will be the main tool necessary to later develop an algorithm for the interpolation of discontinuous functions.


In what follows, even if not always strictly necessary, we will assume that  the kernel~$Φ$ is radial, i.e., of the form~\ref[phi] for some univariate positive definite function~$\phi$.  This applies also to the notations, with the letter~$\phi$ replacing the uppercase letter~$Φ$. Moreover, for the sake of simplicity, $\phi$ will be one of the radial functions introduced in section~\ref[positivesec]. Let $X=\{\range x_N/\}$ be a set of data sites and $f$ a function that is no more assumed to belong to the native space of~$\phi$.  Suppose that $f$ has been sampled at~$X$ and the sampling is dense enough to catch the relevant features of~$f$ on its considered domain.  Our goal is to recognise whether or not $f$ is discontinuous just by looking at~$X$ and the corresponding sampled function values~$f(X)$.
...
$$
whose elements are defined as~$E_k\coloneq f(x_k)-s^{(k)\!}(x_k)$, and which is typically used to find the appropriate shape parameter~$ε$ for the interpolation with  a positive definite radial function~$\phi_ε$, defined as in~\ref[phiepsilon]. The value of~$ε$ is chosen such that the $p$-norm
$$
...
$$
\Bm A^{(k)}\,(C^{[k]}_1,\dots,\rlap{\typoscale[1700/]$\backslash$} C^{[k]}_k,\dots, C^{[k]}_N)^T =  \Bm 0,
$$
...
\preskip
\definition
Given a set~$X = \{\range x_N/\}\subset\Omega\subseteq\R^d$ of data sites, with $N ≥ 2$, a set $F = \{\range f_N/\}$ of function values, and a radial kernel~$\phi$ which is positive definite on~$\R^d$, we define the {\em $\phi$-regularity vector}
$$
...
The regularity is inspected by the kernel function $\phi$, and the tolerance~$\tau$ is the decision boundary between regular (value~$1$) and not regular (value~$-1$) functions, which must be set according to how much evidence is required for the classification. The rationale behind the above given definition for the classifier~$\Cal R$ should become apparent in the following section.
